    ...
    <section class="py-4">
        <div class="container">
            ...
            <div class="row justify-space-between py-2">
                    ...
                    <div class="card card-profile mt-md-0 mt-5">
                        <div class="card-header mt-n4 mx-3 p-0 bg-transparent position-relative z-index-2">
                            <a class="d-block blur-shadow-image">
                                <a href="{{route('admission.show',$item->id)}}"><img src="{{asset('images/admission/'.$item->image)}}" alt="{{$item->name}}}"
                                    class="img-fluid w-100 border-radius-lg" style="max-height: 221px;max-width:348px">
                            </a>
                        </div>
                        <div class="card-body text-center">
                             <h4 class="mb-0"><a class="nav-link " href="{{route('admission.show',$item->id)}}">{{$item->name}}</a></h4>
                            <div class="row justify-content-center text-center">
                                <div class="col-lg-4 col-4">
                                    <h5 class="text-warning mb-2">Edit</h5>
                                    <a href="{{route('admission.edit',$item->id)}}" class="btn btn-warning"><i class="fa fa-pen"></i> </a>
                                </div>
                                <div class="col-lg-4 col-4">
                                    <h5 class="text-danger mb-2">Delete</h5>
                                    <a href="{{route('admission.destroy',$item->id)}}"  class="btn btn-danger" id="delete"><i class="fa fa-trash"></i></a>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                @else
                    <p class="text-center">NO Admission</p>
                @endif
